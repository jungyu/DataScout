...
"""
Financial Times 爬蟲 (關鍵字版)

此模組提供金融時報(Financial Times)新聞網站的爬蟲功能，可以批次搜尋關鍵字組合並支援斷點續爬。
加入反爬蟲偵測與處理，以及處理分頁功能。
"""
...
SITE_CONFIG = {
    ...
    "max_pages": 5  # 替代原本的 load_more_count
}

# 選擇器配置
SELECTORS = {
    "search": {
        "list_item": ".o-teaser", # 新聞列表項
        "title": ".o-teaser__heading", # 標題
        "date": ".o-date", # 發布日期
        "summary": ".o-teaser__standfirst", # 摘要
        "url": ".o-teaser__heading a", # 連結
        "load_more": ".search-pagination__more-button", # 加載更多按鈕
        "count": ".search-results__heading" # 總筆數選擇器
    },
    "article": {
        "title": ".article__title", # 文章標題
        "date": ".article-date", # 文章日期
        "author": ".n-content-tag--author", # 作者
        "content": ".n-content-body p, .n-content-body h2, .n-content-body h3", # 文章內容
        "image": ".n-content-image img" # 圖片
    },
    "paywall": {
        "login_button": ".o-barrier__login", # 登入按鈕
        "paywall": ".barrier", # 付費牆
        "cookie_banner": "#cop-banner", # Cookie 橫幅
        "cookie_accept": "#cop-banner button[data-action='accept']", # 接受 Cookie 按鈕
        "subscription_banner": ".o-message--subscription", # 訂閱橫幅
        "consent_banner": ".o-cookie-message", # 同意橫幅
        "consent_button": ".o-cookie-message button" # 同意按鈕
    }
}
...
# 日誌檔案路徑
LOGS_FILE = Path(SITE_CONFIG["save_path"]) / "news_ft_logs.csv"

# 加載進度記錄檔案
LOAD_MORE_LOGS_FILE = Path(SITE_CONFIG["save_path"]) / "news_ft_load_more_logs.csv"
...
class FTScraper(PlaywrightBase):
    """
    Financial Times 爬蟲類
    繼承自 PlaywrightBase，提供金融時報網站的爬蟲功能
    """

    def __init__(self):
        """初始化 Financial Times 爬蟲"""
    ...
    def search_news(self, keyword: str, save_page: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        try:
            url = f"{self.site_config['search_url']}?q={quote(keyword)}"
            logger.info(f"搜索新聞: {keyword}")
            
            # 先訪問主頁
            logger.info("先訪問 FT 主頁以建立 session")
    ...
    def _check_for_paywall_or_popup(self) -> bool:
        ...
        try:
            ...
            paywall_selectors = [
                ...
                self.selectors["paywall"]["consent_banner"],
                "#barrier-page",
                ".o-cookie-message",
                ".o-banner",
                ".o-message",
            ...
            paywall_keywords = [
                ...
                "paid content", "paywall", "continue reading"
            ]
    ...
    def _handle_paywall_or_popup(self) -> bool:
        ...
        try:
            ...
            # 處理其他彈窗
            popup_selectors = [
                ".o-message__actions button",
                ".n-messaging-banner__close",
                ".n-messaging-banner button",
                ".o-banner__close",
                ".o-message__close"
            ]
            ...
            if self.page.query_selector(self.selectors["paywall"]["paywall"]) or \
               self.page.query_selector(self.selectors["paywall"]["login_button"]):
                ...
                print("\n檢測到 Financial Times 付費牆，請在瀏覽器中手動登入或關閉彈窗。")
                print("如果您有 FT 帳號，請現在登入；如果沒有，請嘗試關閉彈窗或繼續瀏覽。")
        ...
        for load_count in range(1, max_loads + 1):
            try:
                ...
                if not self.page.query_selector(load_more_button):
                    logger.info(f"未找到「Load More」按鈕，可能已到達末頁")
                    break
                
                logger.info(f"點擊「Load More」按鈕 ({load_count}/{max_loads})")
    ...
    def _save_page_content(self, keyword: str):
        """保存頁面內容和截圖"""
        try:
            base_name = f"ft_search_{keyword}"
    ...
    def close(self):
        """關閉瀏覽器"""
        super().close()
        logger.info("Financial Times 爬蟲已關閉")
    ...
    def paginate_results(self, keyword: str, max_pages: int = 5) -> List[Dict[str, Any]]:
        ...
        for page in range(2, max_pages + 1):
            try:
                # 構建帶有頁碼的 URL
                page_url = f"{self.site_config['search_url']}?q={quote(keyword)}&page={page}&sort=relevance&isFirstView=false"
...
def save_results(keyword_pair: List[str], news_list: List[Dict[str, Any]]):
    ...
    try:
        ...
        file_name = f"news_ft_{keyword_str}.json"
...
def log_page_status(keyword: str, page_num: int, count: int, status: str):
    """記錄頁面爬取狀態"""
    try:
        page_logs_file = Path(SITE_CONFIG["save_path"]) / "news_ft_page_logs.csv"
    ...
    try:
        ...
        for idx, keyword_pair in enumerate(keyword_pairs):
            ...
            try:
                ...
                # 使用 URL 參數翻頁獲取更多結果 (替代原本的 load_more_results)
