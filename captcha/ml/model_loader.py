...
class ModelLoader:
    ...
    def detect_objects(self, image: Image.Image, target_class: Optional[str] = None) -> list:
        """使用物件檢測模型檢測圖像中的物體"""
        ...
        try:
            ...
            # 處理預測結果（示例，實際處理方式依賴於模型結構和輸出格式）
            if isinstance(predictions, list) and len(predictions) > 0:
                predictions = predictions[0]  # 批次中的第一個圖像
                
                for i, box in enumerate(predictions['boxes']):
                    score = predictions['scores'][i].item()
                    class_idx = predictions['labels'][i].item()
                    ...
                    # 獲取類別名稱
                    class_name = classes[class_idx - 1] if 0 < class_idx <= len(classes) else f"class_{class_idx}"
