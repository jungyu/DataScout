...
# 複製構建的檔案
echo -e "${YELLOW}複製 JavaScript 檔案...${NC}"
cp -f dist/js/* "$STATIC_DIR/js/dist/"

echo -e "${YELLOW}複製 CSS 檔案...${NC}"
cp -f dist/css/output.css "$STATIC_DIR/css/"

# 複製 favicon 如果存在
if [ -f "public/favicon.ico" ]; then
  echo -e "${YELLOW}複製 favicon...${NC}"
  cp -f public/favicon.ico "$STATIC_DIR/"
fi

echo -e "${GREEN}資源檔案複製完成${NC}"
...
echo -e "${YELLOW}cd web_service && python -m uvicorn app.main:app --reload${NC}"
echo -e ""
