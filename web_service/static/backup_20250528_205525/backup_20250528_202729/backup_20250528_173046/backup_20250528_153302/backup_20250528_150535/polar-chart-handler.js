...
(function() {
    ...
    try {
      ...
      if (window.chartErrorHandler) {          window.chartErrorHandler.showError('polarChart', `無法渲染極區圖: ${error.message}`);
        ...
        const alternativeFiles = [
          '/static/assets/examples/apexcharts_polar_basic.json',
          '/static/assets/examples/apexcharts_polar_investment.json',
          '/static/assets/examples/apexcharts_polar_education.json'
        ];
  ...
  window.loadPolarChartExample = function(exampleFile) {
    ...
    const examplePath = `/static/assets/examples/${modernFileName}`;
